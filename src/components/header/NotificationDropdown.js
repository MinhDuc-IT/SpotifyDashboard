...
export default function NotificationDropdown() {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await getAllNotifications(1, 10);
  ...
  const timeAgo = (createdAt) => {
    ...
    const weeksAgo = Math.floor(timeDiff / 604800000);
    const monthsAgo = Math.floor(timeDiff / 2628000000);
    const yearsAgo = Math.floor(timeDiff / 31536000000);

    if (minutesAgo < 1) {
      return "Vừa xong"; // Thêm điều kiện này cho trường hợp nhỏ hơn 1 phút
    }
    if (minutesAgo < 60) {
  ...
  useEffect(() => {
    ...
    connectToNotificationHub(handleNotification, token);
    fetchNotifications();
  }, [loadData]);
